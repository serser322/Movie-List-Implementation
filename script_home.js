...
let pageContent = [];
let layoutState = 'card';

function renderMovieList(data, layout) {
  let rawHTML = '';
...
searchForm.addEventListener('submit', function onSearchFormSubmitted(event) {
  ...
  pageContent = getMoviesByPage(1);
  renderMovieList(pageContent, layoutState);
});
...
paginator.addEventListener('click', function onPaginatorClicked(event) {
  ...
  pageContent = getMoviesByPage(page);
  renderMovieList(pageContent, layoutState);
});
...
layoutBtn.addEventListener('click', function (event) {
  layoutState = event.target.dataset.layout;
  renderMovieList(pageContent, layoutState);
});
